...
test("게임 만들기 화면은 설명 카드 대신 바로 시작 구조를 보여준다", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "비행" })).toBeVisible();
  await expect(page.getByRole("button", { name: "슈팅" })).toBeVisible();
  await expect(page.getByRole("button", { name: "레이싱" })).toHaveCount(0);
  await expect(page.locator("form button svg")).toHaveCount(2);
});

...
export function QueueSummaryPane({ title, count, events, labelForEvent }: { title: string; count: number; events: Array<{ id: string; event_type: string; summary?: string }>; labelForEvent: (eventType: string) => string; }) {
  return (
    <Card data-admin-surface={title} className="bg-white/72 p-5">
      ...
      <div className="grid gap-3">
        {events.length === 0 ? (
          ...
          events.map((event) => (
            <div key={event.id} className="rounded-[1rem] border border-[#1b1337]/8 bg-white/88 px-4 py-4 text-sm leading-6 text-muted-foreground">

  ...
  return (
    <Card className="rounded-[1.85rem] border-white/8 bg-[#101118]/82 p-5">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <h2 className="font-display text-[1.7rem] tracking-[-0.04em] text-foreground">이벤트 타임라인</h2>
      ...
      <ScrollArea className="h-[28rem] pr-3 xl:h-[40rem]">
        <div className="grid gap-3">
          {events.map((event) => (
            <article key={event.id} className="rounded-[1.4rem] border border-white/8 bg-white/[0.03] p-4">
              ...
              <div className="mt-3 space-y-2 text-sm leading-6 text-muted-foreground">
                ...
                <p className="text-xs uppercase tracking-[0.18em] text-accent">{labelForEvent(event.event_type)}</p>
                ...
                {event.error_code ? <p className="text-red-200">오류 코드: {event.error_code}</p> : null}
              </div>
            </article>
          ))}
          {events.length === 0 ? <p className="rounded-[1.35rem] border border-dashed border-white/10 px-4 py-6 text-sm text-muted-foreground">표시할 이벤트가 없습니다.</p> : null}
        </div>
      </ScrollArea>
    </Card>
  );

...
export function useEditorSession() {
  ...
  const refreshActivitiesFromEvents = useCallback(
    async (sessionId: string) => {
      ...
      const nextActivities = events.map((event) => normalizeActivityFromEvent(event)).filter((row): row is AgentActivity => Boolean(row));
  ...
  const pollRun = useCallback(async (sessionId: string, queuedRunId: string): Promise<RunResponse> => {
    let attempt = 0;
    let transientFailures = 0;
    while (attempt < 180) {
      attempt += 1;
      try {
        const response = await fetch(`${API_BASE}/${sessionId}/runs/${queuedRunId}`, { method: "GET", cache: "no-store" });
        const payload = (await response.json().catch(() => ({}))) as RunResponse;
        if (!response.ok) {
          if (transientFailures < MAX_TRANSIENT_POLL_FAILURES && isTransientCoreEnginePollFailure({ status: response.status, payload })) {
            transientFailures += 1;
        ...
        if (transientFailures < MAX_TRANSIENT_POLL_FAILURES && isTransientCoreEnginePollFailure({ message })) {
          transientFailures += 1;
          setRunStatus("running");
          setRunError(`코어 엔진 재연결 대기 중 (${transientFailures}/${MAX_TRANSIENT_POLL_FAILURES})`);
          await wait(1500);
  ...
  useEffect(() => {
    ...
    localStorage.setItem(LAYOUT_STORAGE_KEY, JSON.stringify({ chatWidth }));
  }, [chatWidth, isDesktop]);
  ...
  useEffect(() => {
    ...
    const restore = async () => {
      ...
      try {
        ...
        const nextActivities = events.map((event) => normalizeActivityFromEvent(event)).filter((row): row is AgentActivity => Boolean(row));
        const eventMessages = events.map((event) => eventToChatMessage(event)).filter((row): row is ChatMessage => Boolean(row));
        ...
        setRunStatus(
          (snapshot.current_run_status && ["idle", "queued", "running", "succeeded", "failed", "cancelled"].includes(snapshot.current_run_status)
  ...
  useEffect(() => {
    const handleMouseMove = (event: MouseEvent) => {
      ...
      if (drag.mode === "chat") {
        ...
        const maxChat = Math.max(CHAT_MIN_WIDTH, Math.min(CHAT_MAX_WIDTH, drag.rectRight - drag.rectLeft - 520));
  ...
  const submitChatIssue = useCallback(
    async ({ sessionId, prompt, attachment }: { sessionId: string; prompt: string; attachment?: ChatAttachment | null }) => {
      setIsIssueBusy(true);
      try {
        pushMessage({ id: makeId(), role: "system", content: "피드백 접수됨 · 최적 에이전트가 수정 방향을 정리하는 중입니다.", timestamp: Date.now() });
        ...
        const issueResponse = await fetch(`${API_BASE}/${sessionId}/issues`, {
          ...
          body: JSON.stringify({
            ...
            image_attachment: attachment
              ? { name: attachment.name, mime_type: attachment.mime_type, data_url: attachment.data_url }
              : undefined,
          }),
        });
        ...
        pushMessage({ id: makeId(), role: "system", content: "수정안 생성 중... 프리뷰를 갱신할 준비를 합니다.", timestamp: Date.now() });
  ...
  const handleSend = useCallback(
    async ({ prompt, attachment, mode = "auto" }: ChatSendPayload) => {
      ...
      try {
        ...
        const shouldUseIssueFlow = mode === "issue" || (mode === "auto" && Boolean(session?.html.trim()));
        ...
        if (planRes.ok) {
          const planPayload = (await planRes.json().catch(() => null)) as { mode?: string; summary?: string; checklist?: string[] } | null;
                ...
                ? {
                    ...
                    messages: [
                      ...prev.messages,
                      {
                        id: makeId(),
                        role: "system",
                        content: `기획 초안(${planPayload.mode ?? "unknown"}): ${planPayload.summary}${checklist ? ` · ${checklist}` : ""}`,
                        timestamp: Date.now(),
                      },
                    ],
                  }
        ...
        setSession((prev) =>
          prev
            ? {
                ...
                messages: [
                  ...prev.messages,
                  { id: makeId(), role: "system", content: `실행 대기열 등록: ${queuedRunId}`, timestamp: Date.now() },
                ],
              }
            : prev,
        );
        ...
        if (runResult.status === "succeeded") {
          ...
          setSession((prev) => {
            ...
            return {
              ...
              status: snapshot?.status ?? "active",
              messages: [...prev.messages, ...activityMessages, { id: makeId(), role: "assistant", content: "실행 완료", timestamp: Date.now() }],
        ...
        const failure = runResult.error_code ? `${runResult.error_code}${runResult.error_detail ? ` · ${runResult.error_detail}` : ""}` : runResult.error_detail || "run_failed";
        setRunError(failure);
        setError(failure);
        setSession((prev) =>
          prev
            ? {
                ...prev,
                messages: [...prev.messages, { id: makeId(), role: "system", content: `실행 실패: ${failure}`, timestamp: Date.now() }],
              }
            : prev,
        );
      } catch (err) {
        ...
        const nextActivities = diagnostics.map((event) => normalizeActivityFromEvent(event)).filter((row): row is AgentActivity => Boolean(row));
        setError(msg);
        setSession((prev) => {
          if (!prev) return prev;
          const diagnosticMessages: ChatMessage[] = diagnostics.slice(-4).map((event) => ({
            ...
            content: `진단 ${event.event_type}: ${event.summary || event.error_code || "상세 없음"}`,
            timestamp: Date.now(),
          }));
          return {
            ...prev,
            messages: [...prev.messages, ...diagnosticMessages, { id: makeId(), role: "system", content: `코어 엔진 응답 오류: ${msg}`, timestamp: Date.now() }],
  ...
  const handleProposeFix = useCallback(async () => {
    ...
    try {
      ...
      setSession((prev) =>
        prev
          ? { ...prev, messages: [...prev.messages, { id: makeId(), role: "assistant", content: `수정안 생성 완료 · proposal_id=${payload.proposal_id} (미리보기 반영)`, timestamp: Date.now() }] }
          : prev,
      );
    ...
    try {
      ...
      setSession((prev) => {
        ...
        return {
          ...prev,
          html: payload.html,
          messages: [...prev.messages, { id: makeId(), role: "system", content: "수정안 적용 완료", timestamp: Date.now() }],
        };
      });
      await refreshActivitiesFromEvents(session.id);
    } catch (err) {
  ...
  const handleRestorePrevious = useCallback(() => {
    ...
    setHtmlHistory((history) => {
      ...
      setSession((prev) => {
        if (!prev) return prev;
        return { ...prev, html: restored, messages: [...prev.messages, { id: makeId(), role: "system", content: "직전 결과를 로컬 프리뷰에 복원했습니다.", timestamp: Date.now() }] };
      });

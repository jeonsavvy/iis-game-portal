...
const EMBED_VIEWPORT_FIX_SCRIPT =
  "<script id=\"iis-embed-viewport-script\">(()=>{const fit=(el)=>{if(!(el instanceof HTMLElement))return;el.style.minHeight='0';el.style.height='100%';el.style.maxHeight='100%';el.style.overflow='hidden';};const apply=()=>{fit(document.documentElement);fit(document.body);const game=(document.getElementById('game')||document.querySelector('canvas'));if(game instanceof HTMLCanvasElement){game.style.width='100%';game.style.height='100%';game.style.maxWidth='100%';game.style.maxHeight='100%';game.style.aspectRatio='auto';let node=game.parentElement;let depth=0;while(node&&depth<5){fit(node);node=node.parentElement;depth+=1;}}const stage=document.querySelector('.stage,.stage-shell,.runtime-root,.game-root,main,[data-overflow-policy]');fit(stage);} ;if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',apply,{once:true});}else{apply();}window.addEventListener('resize',apply,{passive:true});})();</script>";
